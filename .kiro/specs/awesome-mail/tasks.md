...
*   **現狀**: 
    - Client 端已有 Microsoft/Google 整合。
    - Backend `integrations.ts` 路由與服務尚未實作。
*   **Action Items**:
    - [ ] 建立 `awesome-mail/src/routes/integrations.ts`。
...
### 2.5 安全功能補完
- [ ] **PGP 實作**: 將 `PGPKeyManager` 與 `PGPService` 中的 Mock 邏輯替換為真實 OpenPGP 實作 (`openpgp_dart` 等)。
...
- [x] **IMAP Protocol**: `IMAPHandler` 完整實作 (SSL/TLS, Commands, Parsing).
...
- [x] **Platform OAuth**: Windows (System Browser) & Web (GIS) OAuth 服務實作完成。
...
- [x] **資料庫**: Drift, FTS5, Indexes, `ai_task_queue` persistence.
...
- [x] **UI 基礎**: Awesome Design System (macOS), 3-pane Layout, Text Sanitizer (UTF-16 fix), `AppButton`.
...
- [x] **Remote Config**: Feature Flags, Experiments & Maintenance Mode (`RemoteConfigService`), Multi-Environment Support (`EnvironmentConfig`).
